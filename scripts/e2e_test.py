...
"""End-to-end integration test suite verifying the distributed A/B testing system."""
...
def run_e2e_tests() -> None:
    """Execute 8-step full system end-to-end test suite."""
    print("=" * 60)
    ...
    print(f"  ? Seeded experiment verified: ID {exp_data['id']}, Name '{exp_data['name']}'")
    ...
    cached = r_client.hgetall(f"experiment:{seeded['id']}")
    assert cached, f"Experiment {seeded['id']} not found in Redis cache!"
    assert cached["name"] == seeded["name"]
    active_set = r_client.smembers("active_experiments")
    ...
    for i in range(100):
        ...
        res = client.get(
            ...
            params={"user_id": f"user_bench_{i}", "attributes": json.dumps({"country": "US"})},
        )
    ...
    res_99 = client.post(
        "http://localhost:8001/experiments",
        json={
            "name": f"invalid_exp_99_{int(time.time()*1000)}",
            "config": {
    ...
    res_101 = client.post(
        "http://localhost:8001/experiments",
        json={
            "name": f"invalid_exp_101_{int(time.time()*1000)}",
            "config": {
    ...
    unique_exp_name = f"test_e2e_lifecycle_{int(time.time()*1000)}"
    ...
    assert new_exp_res.status_code == 201, f"Failed to create experiment: {new_exp_res.text}"
    ...
    decision_res = client.get(
        "http://localhost:8002/decide",
        params={"user_id": "user_alpha", "attributes": json.dumps({"env": "prod"})},
    ).json()
    ...
    client.patch(
        f"http://localhost:8001/experiments/{exp_id}",
        json={
            "config": {
    ...
    inv_ev = client.post(
        ...
        json={"event_type": "conversion", "payload": {"experiment_id": 1, "variant_name": "control"}},
    )
    assert inv_ev.status_code == 422
    print("  ? Invalid event payload returned 422 Unprocessable Entity")

    # 8. Event Ingestion -> RabbitMQ Queue Publishing
    print("\n[8/8] Testing RabbitMQ Event Publishing...")
    ...
    time.sleep(0.5)
    method_frame, _, body = ch.basic_get(queue="analytics_events", auto_ack=True)
    ...
    print("\n" + "=" * 60)
    print("ALL END-TO-END TESTS PASSED 100% SUCCESSFULLY! [OK]")
    print("=" * 60)
